...
def test(args, img_path, fname):
    # Model name extraction (robust across checkpoint filenames like *_final.pkl)
    wc_file = os.path.basename(args.wc_model_path)
    wc_stem = os.path.splitext(wc_file)[0]
    wc_model_name = wc_stem.replace('_final', '')

    bm_file = os.path.basename(args.bm_model_path)
    bm_stem = os.path.splitext(bm_file)[0]
    bm_model_name = bm_stem.replace('_final', '')
    ...
    def _get_model_with_fallback(model_name: str, n_classes: int, in_channels: int = 3):
        """
        Try to instantiate a model using the requested name; if registry does not contain it,
        fallback to the base token before the first underscore (common in some DewarpNet forks).
        Returns (model, used_name).
        """
        tried = []
        candidates = [model_name]
        if '_' in model_name:
            candidates.append(model_name.split('_', 1)[0])
        for cand in candidates:
            tried.append(cand)
            m = get_model(cand, n_classes, in_channels=in_channels)
            if m is not None:
                return m, cand
        raise RuntimeError(f"Model '{model_name}' not available. Tried: {tried}")

    wc_model, wc_model_name = _get_model_with_fallback(wc_model_name, wc_n_classes, in_channels=3)
    if DEVICE.type == 'cpu':
    ...
    wc_model.eval()
    bm_model, bm_model_name = _get_model_with_fallback(bm_model_name, bm_n_classes, in_channels=3)
    if DEVICE.type == 'cpu':
